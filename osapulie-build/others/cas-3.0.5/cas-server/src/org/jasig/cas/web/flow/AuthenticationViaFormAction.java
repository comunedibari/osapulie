/*
 * Copyright 2005 The JA-SIG Collaborative. All rights reserved. See license distributed with this
...
/**
 * Action to authenticate credentials and retrieve a TicketGrantingTicket for those credentials. If
 * there is a request for renew, then it also generates the Service Ticket required.
 * 
...
public class AuthenticationViaFormAction extends FormAction implements InitializingBean {
	...
	public final Event submit(final RequestContext context) throws Exception {
		...
		final boolean warn = WebUtils.getRequestParameterAsBoolean(request, WebConstants.WARN);
		final boolean renew = WebUtils.getRequestParameterAsBoolean(request, WebConstants.RENEW);
		final String service = WebUtils.getRequestParameterAsString(request, WebConstants.SERVICE);
		final String ticketGrantingTicketIdFromCookie = WebUtils.getCookieValue(request, this.ticketGrantingTicketCookieGenerator.getCookieName());

		if (renew && StringUtils.hasText(ticketGrantingTicketIdFromCookie) && StringUtils.hasText(service)) {

			try {
				final String serviceTicketId = this.centralAuthenticationService.grantServiceTicket(ticketGrantingTicketIdFromCookie, new SimpleService(WebUtils.stripJsessionFromUrl(service)), credentials);
		...
		try {
			...
			ContextUtils.addAttribute(context, AbstractLoginAction.REQUEST_ATTRIBUTE_TICKET_GRANTING_TICKET, ticketGrantingTicketId);
			setWarningCookie(response, warn);
			return success();
		}
		catch (final TicketException e) {
	...
	private final void setWarningCookie(final HttpServletResponse response, final boolean warn) {
		if (warn) {
	...
	/**
	 * Set a CredentialsBinder for additional binding of the HttpServletRequest to the Credentials
	 * instance, beyond our default binding of the Credentials as a Form Object in Spring WebMVC
	 * parlance. By the time we invoke this CredentialsBinder, we have already engaged in default
	 * binding such that for each HttpServletRequest parameter, if there was a JavaBean property of
	 * the Credentials implementation of the same name, we have set that property to be the value of
	 * the corresponding request parameter. This CredentialsBinder plugin point exists to allow
	 * consideration of things other than HttpServletRequest parameters in populating the
	 * Credentials (or more sophisticated consideration of the HttpServletRequest parameters).
	 */
	...
	public final void afterPropertiesSet() throws Exception {
		...
		if (this.getFormObjectClass() == null) {
			...
			logger.info("FormObjectClass not set.  Using default class of " + this.getFormObjectClass().getName() + " with formObjectName " + this.getFormObjectName() + " and validator " + this.getValidator().getClass().getName() + ".");
		}
		...
		if (this.credentialsBinder != null && !this.credentialsBinder.supports(this.getFormObjectClass())) {
			throw new IllegalStateException("CredentialsBinder does not support supplied FormObjectClass: " + this.getClass().getName());
		}
	}
}
