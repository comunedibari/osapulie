...
public class PortaDelegataComuni {
	...
	public void mostraTrattamentiContesto() {
		...
		if ((ltcOUT == null) || (ltcOUT != null && ltcOUT.size() <= 0)) {
			...
			while (i.hasNext()) {
				...
				// preleva le proprietï¿½ del trattamento
	...
	private Busta costruisciBusta(String richiesta, String servizio, String url) throws Exception {
		...
		try {
			 ...
			 * collaborazione � impostato a EGOV_IT_ServizioSincrono - il profilo di trasmissione �
			...
			// primo metodo:
			// - Il progressivo � generato internamente
			// - Il codice amministrazione � ricavato dal parametro 'CodiceAmministrazione' del
			// contesto OUT
			// - Il codice porta � ricavato dal parametro 'CodicePorta' del contesto OUT
			idBusta = pd.generaID();
			...
			dmvo.setTitolo("Richiesta Dati Anagrafici");
	...
	public synchronized String esegui(String richiesta, String servizio, String url) throws Exception {
		...
		if (urlUDDI == null) {
			System.out.println(" - UDDI � disabilitato");
		}
		else {
			System.out.println(" - UDDI � abilitato: " + urlUDDI);
		}
