		...
		<table style="width: 97%;">
			...
			<tr>
				<td colspan="2">
					<fieldset>
						<legend>
							avente titolo alla presentazione della <b>DENUNCIA DI INIZIO ATTIVITA'</b> in quanto:
						</legend>
			...
			<tr>
				<td colspan="2" STYLE="text-align: center;"><strong>COMUNICA :</strong></td>
			</tr>
			<tr>
				<td colspan="2">l'inizio della seguente attività edilizia, trascorsi gg. 30 dalla
								data di acquisizione della presente al protocollo comunale:
				</td>
			</tr>
			<tr>
				<td colspan="2">
			...
			<tr>
				<td colspan="2"><strong>Consapevole che le dichiarazioni mendaci e le falsità in atti comportano 
								le sanzioni previste dall'art. 76 del d.P.R. 445/2000 e la decadenza dai 
								benefici conseguiti con il provvedimento emanato sulla base della dichiarazione 
								non veritiera;</strong>
				</td>
			</tr>
			<tr>
				...
				<td colspan="2">
					<ol style="list-style:lower-alpha outside none;">
						...
						<c:forTokens items="${valoriCampoOL}" delims="," var="tokenName" varStatus="listCnt">
							...
							<c:if test="${listCnt.count==4}"><!-- lettera d) -->
								...
								<table style="width: 97%;">
									<c:forTokens items="${valoriCampo}" delims="," var="tokenName" varStatus="list1chk">
										...
										<tr>
											<td <c:if test="${list1chk.count>=13}"> colspan="3"  </c:if> >
												<input type="checkbox" name="${nomeCampoChk3 }" value="${tokenName}" ${check} />&nbsp;${tokenName}<br/>
												<c:if test="${list1chk.count==15}">
													<c:set var="nomeCampoTxt" value="campiaggiuntivi[48].valore" scope="page" />
													&nbsp;<form:input id="text${nomeCampoTxt }" path="${nomeCampoTxt }" size="80"/>
													&nbsp;<form:errors path="${nomeCampoTxt }" cssStyle="color:red"/>
												</c:if>
							...
							<c:if test="${listCnt.count==5}"><!-- lettera e) -->
								<c:set var="nomeCampoRdb3" value="campiaggiuntivi[96].valore" scope="page"/>
								<c:set var="valoriCampoRdb3" value="${datiPratica.campiaggiuntivi[96].campiAggiuntivi.listaValori}" scope="page"/>
								<c:set var="valoreCampoRdb3" value="${datiPratica.campiaggiuntivi[96].valore}" scope="page"/>
								<c:forTokens items="${valoriCampoRdb3}" delims="," var="tokenName" varStatus="list2chk">
									<input type="radio" name="${nomeCampoRdb3 }" value="${tokenName}" <c:if test="${valoreCampoRdb3 == tokenName}">checked</c:if> />
									&nbsp;${tokenName}
									<c:if test="${list2chk.count==2}">
										<c:set var="nomeCampo" value="campiaggiuntivi[49].valore" scope="page" /> 
										<b>${datiPratica.campiaggiuntivi[49].campiAggiuntivi.label}: </b>
										<%@ include file="../common/fielddate.jsp"%>
									</c:if>
									<br/>
								</c:forTokens>
								<form:errors path="${nomeCampoRdb3 }" cssStyle="color:red"/>
							</c:if>
							<c:if test="${listCnt.count==6}"><!-- lettera f) -->
							...
							<c:if test="${listCnt.count==12}"><!-- lettera m) -->
								&nbsp;
								<c:set var="nomeCampo" value="campiaggiuntivi[60].valore" scope="page" />
								<%@ include file="../common/fieldtextarea.jsp"%>
								<br/>
							</c:if>
							<c:if test="${listCnt.count==13}"><!-- lettera n) -->
								&nbsp;
							...
							<c:if test="${listCnt.count==14}"><!-- lettera o) -->
								&nbsp;
								<c:set var="size" value="66" scope="page" /> 
							...
							<c:if test="${listCnt.count==15}"><!-- lettera p) -->
								&nbsp;
								<c:set var="size" value="66" scope="page" /> 
