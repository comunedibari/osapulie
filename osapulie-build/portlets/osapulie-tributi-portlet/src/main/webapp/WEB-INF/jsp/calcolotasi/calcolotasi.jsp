...
<portlet:actionURL var="calcoloTasiTotaleUrl">
	<portlet:param name="ope" value="calcoloTasiTotale" />
</portlet:actionURL>

<portlet:actionURL var="calcoloTasiUrl">
	<portlet:param name="ope" value="dichiarazioneReport" />
</portlet:actionURL>
...
<c:set var="idForm" value="calcoloTasi" />
<c:set var="dati" value="${datiCalTasi}" />
<c:set var="numeroImmobiliDaVisura" value="${datiCalTasi.numeroImmobiliDaVisura}" />

<div class="mainDiv">
	<c:choose>
		<c:when test="${!empty datiCalTasi}">
			<form:form id="${idForm}" action="${calcoloTasiTotaleUrl}" method="post" commandName="datiCalTasi" >
			
			<form:hidden id="flagNuovoImmobile" path="nuovoImmobile" />
						
				<c:if test="${empty download}">
					<%@ include file="../common/calcoloTasiDatiAnagrafici.jsp"%>				
					<fieldset>
						<legend>
						...
						<div id="contenitoreTableRiassunto">
							<table class="genericTable elencoRisultati">
								...
								<c:forEach items="${datiCalTasi.datiRiepilogo}" var="datoRiepilogo" varStatus="i">
									...
									<tr>
										...
										<td>
											<a href="${eliminaURL}" title="Elimina"  onclick="javascript:return(confirm('Vuoi eliminare l immobile?'))"><spring:message code="label.rimuovi"/></a>
							...
							<c:out value="${datiCalTasi.totale}"></c:out>
					 	</div>		
					</fieldset>
					...
					<div id="contenitoreDettagli">
						<c:if test="${not empty datiCalTasi.datiRiepilogo}">
					<c:forEach items="${datiCalTasi.datiRiepilogo}" var="datoRiepilogo" varStatus="status">
									...
									<tr>
									 ...
									 <tr>
										...
										<tr>
											<td colspan="6" align="center">
													<label>
														<spring:message code="label.calcoloTasi" />
													</label>
												</td>
											</tr>
											<tr>
									   ...
									   <!-- Sezione relativa alle pertinenze  -->
										...
										<table class="genericTable">	
											<tr>
												...
												<td colspan="5">
													<form:input path="datiRiepilogo[${status.index}].impostaTasi" size="8" readonly="true" style="background-color:#DDDDDD" />
													<div><form:errors path="datiRiepilogo[${status.index}].impostaTasi" cssStyle="color:red"/></div>
												</td>
											</tr>
											<tr>
												<td>
				...
				<div id="contenitoreFormInserimento" style="display:none">
					<%@ include file="../calcolotasi/formImmobile.jsp"%>
				</div>	
				</c:if>
			</form:form>
		</c:when>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		/*al caricamento della pagina, disabilito il pulsante per la generazione dell''F24 se non è stato censito
		almeno un immobile
		*/
		...
		/*quando carica la pagina, controllo se il flagNuovoImmobile è settato a true. Se è così, 
	...
	* TRUE=il pulsante è disabilitato
	*  FALSE= il pulsante è abilitato
	*/
	...
	/*Questo metodo effettua una chiamata ajax per recuperare le liste più
	...
	function setLists(id,valoreSelectCategoria){
		...
		console.log(valoreSelectCategoria);
