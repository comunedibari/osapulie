<form:form modelAttribute="uploadItem" action="${uploadDichiarazioneUrl}" method="post"	enctype="multipart/form-data" id="formUpload">
	<fieldset>
		<legend>
		...
		<c:choose>
			<c:when test="${selectNumAllegati != null}">	
				...
				<div class="container_pulsante">
					<input id="inviaDichiarazioneButton" type="submit" name="invia" value="Invia" onclick="this.disabled=true;$('#formUpload').submit();"/>
			...
			<c:otherwise>	
				<div class="descriptionDiv">
					...
					<ul>
						<c:if test="${fn:length(fn:trim(datiTari.allegatoUno)) != 0}"><li>- ${datiTari.allegatoUno}</li></c:if>
						<c:if test="${fn:length(fn:trim(datiTari.allegatoDue)) != 0}"><li>- ${datiTari.allegatoDue}</li></c:if>
						<c:if test="${fn:length(fn:trim(datiTari.allegatoTre)) != 0}"><li>- ${datiTari.allegatoTre}</li></c:if>
						<c:if test="${fn:length(fn:trim(datiTari.allegatoQuattro)) != 0}"><li>- ${datiTari.allegatoQuattro}</li></c:if>
						<c:if test="${fn:length(fn:trim(datiTari.allegatoUno)) == 0 && fn:length(fn:trim(datiTari.allegatoDue)) == 0 && fn:length(fn:trim(datiTari.allegatoTre)) == 0 && fn:length(fn:trim(datiTari.allegatoQuattro)) == 0}"><li>- Nessuno</li></c:if>
					</ul>
				</div>
				<br/>
