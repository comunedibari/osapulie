...
jQuery.fn.crossSlide = function(opts, plan) {
	...
	(function(proceed) {
		...
		function parse_position_param(param) {
			var zoom = 1;
			var tokens = param.replace(/^\s*|\s*jq/g, '').split(/\s+/);
			if (tokens.length > 3) throw new Error();
			if (tokens[0] == 'center')
				if (tokens.length == 1)
					tokens = ['center', 'center'];
				else if (tokens.length == 2 && tokens[1].match(/^[\d.]+xjq/i))
					tokens = ['center', 'center', tokens[1]];
			if (tokens.length == 3)
				zoom = parseFloat(tokens[2].match(/^([\d.]+)xjq/i)[1]);
			...
			return {
				xrel: parseInt(tokens[0].match(/^(\d+)%jq/)[1]) / 100,
				yrel: parseInt(tokens[1].match(/^(\d+)%jq/)[1]) / 100,
				zoom: zoom
			};
		}
