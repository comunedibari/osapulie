...
function registroAudit(){
	...
	this_.table=$('#tblSearchAudit').DataTable({
		data : data,
		pageLength: 25,
        "language" : {
            "url" : $('#lang_').val()
		...
		columnDefs: [ {
		    ...
		    render: function ( data, type, row, meta ) {
		    	...
		    	return row['flagGenerato']=='1'? this_.templateProtocolloBtnOpen(row) : (this_.templateProtocolloBtnRigenera(row));//'<a href="javascript:void(0)" onclick=\'regAudit.modal.setMessage("'+data+'").open();\'>Download</a>';
		    }
		  } ]
...
registroAudit.prototype.searchProtocolli=function(){
	var _this=this;
	...
	$.ajax({
            ...
            url: "/osconservazione/rest/protected/regaudits.json",
            data: {from:regAudit.dataFrom(),to:regAudit.dataTo(),__:new Date().getTime()},
            dataType: "json",
...
registroAudit.prototype.download=function(url_){
	var this_=this;
...
registroAudit.prototype.genera=function(data){
...
registroAudit.prototype.message=function(key){
	var this__=this;
...
registroAudit.prototype.language={
		it:{
...
var regAudit = new registroAudit();

$(document).ready(function() {
regAudit.initTable([]);
$("#search-proto").bind("click",function(){regAudit.searchProtocolli()});
